...
#SBATCH --mem=48G
#SBATCH --output=logs/slurm_agent2_%j.out
...
echo "[$(date)] WORKDIR=${WORKDIR}  agent=2 (SAC-ShrinkPerturb)"

run_one() {
    local seed=$1 gpu=$2
    echo "[$(date)] START agent=2 seed=${seed} gpu=${gpu}"
    ...
    singularity exec --disable-cache --nv \
        ...
        bash -c "
            ...
            python maniskill_vit/src/run_experiments.py \
                ...
                --ckpt_suffix _full_seed${seed}_agent2
        " > logs/agent2_seed${seed}.log 2>&1
    echo "[$(date)] DONE  agent=2 seed=${seed} gpu=${gpu}"
}
...
echo "[$(date)] All seeds complete (agent 2)."
